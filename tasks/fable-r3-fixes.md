...
- **Legal (compliance) — KLART (Erik gav ok 2026-06-13, commits 0cf22cc + a684422):** Privacy Shield→DPF i alla 5 privacy-policies (#310-315); HTML-entities→UTF-8 i alla 10 legal-filer (~1950 st, &gt; behållna som breadcrumb-separator); klagomåls-myndighet-fallback "din nationella DPA" tillagd i sv/da/no/fi (#302, EN hade den redan). Lagval/tvist (svensk lag+ARN per locale) + tillsynsmyndigheter (IMY/Datatilsynet/Tietosuoja) granskade = korrekta, ingen ändring.
  - **KVAR = beslut (ej mekaniskt):** (1) bumpa "Senast uppdaterad 2026-02-09" + ev. notifiera användare om DPF-ändringen = compliance-beslut → Erik; (2) #160 LocationMessage klartext-koordinater = designbeslut → Erik (visuellt, fråga-först-regeln).
...
- **Batch L3 timers (36f93f3):** ShareTargetHandler/ForwardPicker focus-timeout cleanup, OnboardingSlides anim-timer, MessageBubble heart-timer.
- **Batch a11y/i18n reused-keys (1e87cc7):** IncomingCallModal answer/decline aria, LocationMessage role=button, MessageReactions aria+pressed, TrialBanner close, EmojiPicker redundant-listener+placeholder, InlineReactionBar Escape+aria, PollCreator icon-aria.
- **Batch derived-state (e503d44):** LegalPage html useMemo, NewChat filteredContacts useMemo, ZemiNumberInput drop displayValue, OwnerApprovals totalCount derived.
- **Batch i18n new-keys (483a6fc):** common.justNow ×5 + WallComments/WallPost; OwnerChatView/Dashboard alt-text.
- **Batch perf/correctness (dff6aa6):** ChatList formatLastMessageTime kalenderdag-bug, CallHistoryItem outgoing-fallback + memo, VideoTile memo, Calls handleCallback useCallback.
- **Batch autofill/no-root (522affa):** Login autocomplete, MFASetup/Verify one-time-code + aria, main.tsx no-root-fallback.
- **Batch L-formatSeconds (commit 879d243):** delad `formatSeconds`-util i utils/datetime; dedup av 5 m:ss-formaterare (VoiceMessage, VoiceRecorder, CallHeader, CallPiP, CallLogMessage).
- **Batch L-avatar (commit b8c522c):** FriendRequestCard + OwnerApprovals (texter+requester) → getInitial/getAvatarColor + getOptimizedAvatarUrl(48) (konsekvent färgad initial + rätt bildstorlek, #769/#566/#565).
- **Batch L-a11y2 (commit a356f29):** ImageMessage fullscreen close/save/share + galleri-pilar aria-labels (nya a11y.previousImage/nextImage ×5), ChatList arkiv-toggle aria-expanded, MemberLimitDialog rader role=checkbox + tangentbord (Enter/Space).
- **Batch L-correctness2 (commit 99e03f8):** ErrorBoundary retry-tak (3, sen bara "till start"), TrialBanner hourly-tick mot stale daysLeft över midnatt, ThemeContext native StatusBar sync mot temat (hslStringToHex).
- **Batch L-deadcode/debounce (commit fabd46e):** raderat oanvänd Home.{tsx,css}; ChatView markChatAsRead debounce (800ms) vid meddelandeskur.
- **~78/177 LÅG KLARA** (+ CSS-extraktion-svansen medvetet skippad som kosmetisk, se nedan). Build-verifierad. KVAR (~99, mest kosmetisk CSS + legal Hink C): stora teman = CSS-extraktion-svep forts. (OnboardingSlides/ConfettiAnimation/ReportButton/ChoosePlan/Dashboard/NewChat/Friends/ChatList/OwnerChatView/OwnerApprovals/TillkallaAlertCard/WallComments/MFAVerify/MFASetup), a11y aria-labels + i18n-hårdkodning (många, svep), kvarvarande timeout-läckor (OnboardingSlides/ShareTargetHandler/ForwardPicker/MessageBubble-heart), derived-state→useMemo (NewChat/Friends/OwnerApprovals totalCount/LegalPage), shared <UserAvatar>-komponent (4+ dubbletter — KONSEKVENS-buggarna nu fixade via utils i b8c522c; full komponent-extraktion återstår men är ren dedup/kosmetik, hög kollisionsrisk, låg prio), legal-typos/Privacy-Shield/entities (= Hink C, Erik), småperf (optimistic updates, getOptimizedAvatarUrl i GroupAvatar). Inga av dessa är blockerande; betas av löpande.
- **Batch L-css-leaks (commits 5b36789, 8ab87d0):** scopat läckande globala selektorer (Calls ion-segment, MFASetup/Verify bara h2, Support ion-accordion, QuickMessageManager ion-reorder, ChatSearchModal `ion-modal ...` som stylade ALLA modaler) + döpt om kolliderande globala `@keyframes` (pulse ×3 olika defs, typing-bounce, fadeIn — PollCreator använde odefinierad fadeIn). **Keyframes-kollisionerna var äkta buggar** (sist monterad kapade andras animation).
- **CSS-extraktion-analys (BESLUT, ej extraherat):** ~70 filer har inline `<style>`. Generiska klassnamn (.section-title ×9, .avatar-placeholder ×10, .loading-state ×11, .empty-state, .card, .error-text m.fl.) är globala men **ofarliga**: sidor är route-monterade (en `<style>` live i taget). Overlay-komponenter (IncomingCallModal/ChatSearchModal/CreateTexterModal) har avvikande defs men är fullskärm → ingen visuell kollision. Full CSS-modul-extraktion = ren maintainability, hög kollisionsrisk vid global→scoped-flytt, noll runtime/UX-vinst (React återinjicerar ej identisk `<style>`-text). **Skippas medvetet** (no-overengineering); äkta vinster (list-renderade komponenter, läckande selektorer, keyframes) redan tagna.
- **Beslut/skip:** ForwardPicker "store-source" (#) — appen har ingen global chats-store (ChatList äger egen state); färsk getMyChats vid öppning är korrekt beteende, en delad store vore overkill → skippas. markChatAsRead-debounce + Home-deadcode + ErrorBoundary-retry nu KLARA (fabd46e/99e03f8).
